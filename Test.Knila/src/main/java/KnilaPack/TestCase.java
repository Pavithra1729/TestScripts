...
import webPageobjects.AssertVerify;
...
public class TestCase extends CommonFunctions{
	...
	public void RegisterTestCase() throws IOException, InterruptedException
	{
		...
		PageFactory.initElements(driver, AssertVerify.class);
		
		  String FLName =
				  AssertVerify.FLNameVerify.getText();

		  System.out.println(FLName);
		
		  String GenderVerify =
				  AssertVerify.GenderVerify.getText();
		System.out.println(GenderVerify);
		
		String Date =
				AssertVerify.DateVerify.getText();
		System.out.println(Date);
		
		String Address =
				AssertVerify.AddressVerify.getText();
		
		System.out.println(Address);
		
		String Phone =
				AssertVerify.PhoneNoVerify.getText();
	...
	public void StartVisitTestCase() throws AWTException, IOException
	{
        ...
        String UploadSuccess = driver.findElement(By.xpath("//div[@class='toast-item-close']//following::p")).getText();
        
        System.out.println(UploadSuccess);
		
		String ExpectedStatement = "The attachment was successfully uploaded.";
		
		if (UploadSuccess.contains(ExpectedStatement)) 
		{
			Test.log(Status.PASS,"Successfully attachments are added to patient details"); 
		}
		
		else
		{
			Test.log(Status.FAIL,"Attachment is not added properly");
		}
	...
	@Test(priority=4)
	public void PatientRecordSearch() throws IOException, InterruptedException
	{
		...
		String RecordMatch =
				PatientRecordObjects.MatchRecord.getText();

		System.out.println(RecordMatch);
		
		String ExpectedMatch = "No matching records found";
		
		if(RecordMatch.contains(ExpectedMatch))
		{
		...
		{
			Test.log(Status.FAIL,"Patient details are not deleted");
		}
